...
const config: AuthConfig = {
  appId: import.meta.env.VITE_APP_ID || 'learning',
  appName: import.meta.env.VITE_APP_NAME || 'Learning',
...
export async function checkAuth(): Promise<boolean> {
  try {
    ...
    const url = '/api/v1/core/auth/center/me/'
    console.log('[Auth] Checking auth at:', url)
    ...
    console.log('[Auth] checkAuth response:', response.status, response.statusText)
    return response.ok
  } catch (error) {
...
export async function logout(): Promise<void> {
  try {
    await fetch('/api/v1/core/auth/center/logout/', {
      method: 'POST',
      credentials: 'include',
      headers: {
...
export async function refreshToken(): Promise<string | null> {
  try {
    const response = await fetch(
      '/api/v1/core/auth/center/token/refresh/',
      {
        method: 'POST',
        credentials: 'include',
      }
