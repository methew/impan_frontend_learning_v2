...
function parseJSON(text: string): ImportItem[] {
  ...
  if (data.vocab || data.data) {
    return data.vocab || data.data
  }
  return [data]
}
...
export function VocabImportDialog({
  ...
  // 创建词汇树
  async function createVocabTree(item: ImportItem): Promise<void> {
    // 创建词条根节点
    const termNode = await api.createVocabNode({
      content: item.term,
      node_type: 'term',
    ...
    // 创建读音节点
    if (item.reading) {
      const readingNode = await api.createVocabNode({
      ...
      // 创建义项节点
      if (item.meaning) {
        const senseNode = await api.createVocabNode({
          content: item.meaning,
          node_type: 'sense',
          parent: readingNode.id,
          sort_order: 0,
          is_active: true,
        })
        ...
        if (item.examples && item.examples.length > 0) {
          for (const ex of item.examples) {
            const exampleNode = await api.createVocabNode({
              content: ex.sentence,
              node_type: 'example',
              parent: senseNode.id,
              sort_order: 0,
  ...
  const handleImport = async () => {
    ...
    if (importResult.failed === 0) {
      toast.success(`成功导入 ${importResult.success} 个词汇`)
      onImportComplete()
    } else {
  ...
  const jsonExample = `[
  {
    ...
    "examples": [
      ...
      { "sentence": "朝ご飯を食べる", "translation": "吃早餐" }
    ]
  },
  {
    "term": "飲む",
    ...
    "level": 5,
    "examples": [
      { "sentence": "水を飲む", "translation": "喝水" }
    ]
  }
]`

  const csvExample = `term,reading,meaning,pos,level
食べる,た・べる[2],吃,动词,5
飲む,の・む[1],喝,动词,5
見る,み・る[2],看,动词,5
聞く,き・く[0],听,动词,5`
  ...
  const sampleData = [
    ...
    },
    {
      ...
      level: 5,
      examples: [
        { sentence: "映画を見る", translation: "看电影" },
      ]
    },
    {
      term: "学生",
  ...
  const downloadSample = (format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      ...
      filename = 'vocab_sample.json'
      mimeType = 'application/json'
    } else {
      const headers = 'term,reading,meaning,pos,level\n'
      const rows = sampleData.map(item => 
        `${item.term},${item.reading},${item.meaning},${item.pos},${item.level}`
      ).join('\n')
      content = headers + rows
      filename = 'vocab_sample.csv'
      mimeType = 'text/csv'
    }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
          ...
          <TabsContent value="json" className="flex-1 flex flex-col gap-2 mt-2">
            ...
            <p className="text-xs text-muted-foreground">
              支持数组格式，每个对象包含 term, reading, meaning, pos, level, examples 字段
            </p>
          </TabsContent>
          ...
          <TabsContent value="csv" className="flex-1 flex flex-col gap-2 mt-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              支持 CSV/TSV 格式，第一行为表头，列名: term, reading, meaning, pos, level
            </p>
          </TabsContent>
        </Tabs>
