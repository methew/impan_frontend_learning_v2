...
function parseJSON(text: string): ImportItem[] {
  ...
  if (data.texts || data.data || data.items || data.lessons) {
    return data.texts || data.data || data.items || data.lessons
  }
  return [data]
}
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    headers.forEach((header, index) => {
      ...
      switch (header.toLowerCase()) {
        ...
        case 'time':
        case '时长':
          item.duration = parseInt(value) || 45
          break
      }
    })
...
export function TextImportDialog({
  ...
  async function createTextTree(item: ImportItem): Promise<void> {
    // 创建课文根节点（term 类型）
    const termNode = await api.createTextNode({
      content: item.title,
      node_type: 'term',
      difficulty_level: item.level || 5,
      estimated_duration: item.duration || 45,
      parent: null,
      sort_order: 0,
      is_active: true,
    })
    ...
    // 如果有章节，创建章节节点
    if (item.section) {
      const sectionNode = await api.createTextNode({
      ...
      // 如果有正文内容，创建句子节点
      if (item.content) {
        const sentenceNode = await api.createTextNode({
  ...
  const handleImport = async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      try {
        await createTextTree(item)
        importResult.success++
      } catch (error: any) {
        importResult.failed++
        importResult.errors.push(`${item.title}: ${error.message}`)
      }
    ...
    if (importResult.failed === 0) {
      toast.success(`成功导入 ${importResult.success} 个课文`)
      onImportComplete()
    } else {
  ...
  const jsonExample = `[\n  {\n    "title": "第1课 我的家族",\n    "section": "对话",\n    "section_type": "dialogue",\n    "content": "私は田中です。家族は四人です。父と母と妹と私です。",\n    "translation": "我是田中。我家有四口人。爸爸、妈妈、妹妹和我。",\n    "level": 5,\n    "duration": 45\n  },\n  {\n    "title": "第2课 我的学校",\n    "section": "课文",\n    "section_type": "text",\n    "content": "私の学校は東京にあります。学生が五百人います。",\n    "translation": "我的学校在东京。有五百名学生。",\n    "level": 5,\n    "duration": 50\n  }\n]`

  const csvExample = `title,section,section_type,content,translation,level,duration
第1课 我的家族,对话,dialogue,私は田中です。家族は四人です。,我是田中。我家有四口人。,5,45
第2课 我的学校,课文,text,私の学校は東京にあります。,我的学校在东京。,5,50
第3课 购物,对话,dialogue,すみません、この本はいくらですか。,请问，这本书多少钱？,5,40`
  ...
  const downloadSample = (format: 'json' | 'csv') => {
    ...
    if (format === 'json') {
      ...
      filename = 'text_sample.json'
      mimeType = 'application/json'
    } else {
      const headers = 'title,section,section_type,content,translation,level,duration\n'
      const rows = sampleData.map(item => 
        `${item.title},${item.section},${item.section_type},${item.content},${item.translation},${item.level},${item.duration}`
      ).join('\n')
      content = headers + rows
      filename = 'text_sample.csv'
      mimeType = 'text/csv'
    }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
          ...
          <TabsContent value="json" className="flex-1 flex flex-col gap-2 mt-2">
            ...
            <p className="text-xs text-muted-foreground">
              支持数组格式，每个对象包含 title, section, section_type, content, translation, level, duration 字段
            </p>
          </TabsContent>
          ...
          <TabsContent value="csv" className="flex-1 flex flex-col gap-2 mt-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              支持 CSV/TSV 格式，第一行为表头，列名: title, section, section_type, content, translation, level, duration
            </p>
          </TabsContent>
        </Tabs>
