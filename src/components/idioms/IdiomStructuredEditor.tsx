...
const NODE_TYPE_CONFIG: Record<string, { label: string; icon: any; color: string; description?: string; allowedChildren?: string[] }> = {
  term: { 
    ...
    color: 'bg-rose-500',
    description: '惯用语根节点',
    allowedChildren: ['literal_meaning', 'usage_context', 'example', 'note']
  },
  literal_meaning: { 
    label: '字面意思', 
    icon: BookOpen, 
...
interface IdiomStructuredEditorProps {
  open: boolean
...
export function IdiomStructuredEditor({
  open,
  onOpenChange,
  rootNode,
  onSave,
}: IdiomStructuredEditorProps) {
  ...
  const loadTree = async (rootId: string | number) => {
    setIsLoading(true)
    console.log('[IdiomEditor] Loading tree for node:', rootId)
    try {
      const rootData = await api.getIdiomNodeTree(rootId)
      console.log('[IdiomEditor] Loaded data:', rootData)
      if (!rootData) {
        toast.error('加载惯用语树失败', { description: '返回数据为空' })
        return
      }
      setTree(convertApiNode(rootData))
    } catch (error: any) {
      console.error('[IdiomEditor] Load error:', error)
      toast.error('加载惯用语树失败', { 
  ...
  const handleSave = async () => {
    ...
    try {
      if (tree.isNew) {
        ...
        await api.updateIdiomNode(tree.id, {
          content: tree.content,
          sort_order: tree.order,
        })
      }
      
      toast.success(rootNode ? '惯用语已更新' : '惯用语已创建')
      onSave()
      onOpenChange(false)
    } catch (error: any) {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh]">
          <div className="flex flex-col items-center justify-center py-10 text-center">
            <p className="text-muted-foreground mb-4">无法加载惯用语数据</p>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {rootNode ? `编辑惯用语: ${rootNode.content}` : '新建惯用语'}
          </DialogTitle>
          <DialogDescription>
            按照结构编辑惯用语信息：惯用语 → 字面意思 → 使用场景 → 例句
          </DialogDescription>
        </DialogHeader>
