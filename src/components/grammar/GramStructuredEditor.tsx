...
const NODE_TYPE_CONFIG: Record<string, { label: string; icon: any; color: string; description?: string; allowedChildren?: string[] }> = {
  term: { 
    ...
    color: 'bg-cyan-500',
    description: '语法根节点',
    allowedChildren: ['structure', 'example', 'note']
  },
  structure: { 
    label: '结构', 
    icon: BookOpen, 
...
interface GramStructuredEditorProps {
  open: boolean
...
export function GramStructuredEditor({
  open,
  onOpenChange,
  rootNode,
  onSave,
}: GramStructuredEditorProps) {
  ...
  const loadTree = async (rootId: string | number) => {
    setIsLoading(true)
    console.log('[GramEditor] Loading tree for node:', rootId)
    try {
      const rootData = await api.getGramNodeTree(rootId)
      console.log('[GramEditor] Loaded data:', rootData)
      if (!rootData) {
        toast.error('加载语法树失败', { description: '返回数据为空' })
        return
      }
      setTree(convertApiNode(rootData))
    } catch (error: any) {
      console.error('[GramEditor] Load error:', error)
      toast.error('加载语法树失败', { 
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (tree.isNew) {
        ...
        await api.updateGramNode(tree.id, {
          content: tree.content,
          sort_order: tree.order,
        })
      }
      
      toast.success(rootNode ? '语法已更新' : '语法已创建')
      onSave()
      onOpenChange(false)
    } catch (error: any) {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh]">
          <div className="flex flex-col items-center justify-center py-10 text-center">
            <p className="text-muted-foreground mb-4">无法加载语法数据</p>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {rootNode ? `编辑语法: ${rootNode.content}` : '新建语法'}
          </DialogTitle>
          <DialogDescription>
            按照结构编辑语法信息：条目 → 结构 → 解释 → 例句
          </DialogDescription>
        </DialogHeader>
